...
const action = {
  ...
  getInvoiceKiotViet: createAsyncThunk(
    ...
    async ({ page, perPage, filter }: {
      page: number,
      perPage: number, 
      filter: { idStore: string, supplierId?: string, status?: string, categoryId?: string, categoryId1: string, categoryId2: string, categoryId3: string } }) => {
...
export const InvoiceKiotVietFacade = () => {
  ...
  return {
    ...
    get: ({ page, perPage, filter }: { page: number, perPage: number, filter: { idStore: string, supplierId: string, status: string, categoryId: string, categoryId1: string, categoryId2: string, categoryId3: string } }) => dispatch(action.getInvoiceKiotViet({ page, perPage, filter })),
  };
};
